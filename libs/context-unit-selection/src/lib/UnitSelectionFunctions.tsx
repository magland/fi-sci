...
export const allUnitSelectionState = (s: {
  ...
  if (!s.visibleUnitIds || s.visibleUnitIds.length === 0 || s.selectedUnitIds.size !== s.visibleUnitIds.length)
    return 'partial';
...
export const plotElementClick = (
  ...
  const action = {
    type: evt.shiftKey ? TOGGLE_RANGE : evt.ctrlKey ? TOGGLE_UNIT : UNIQUE_SELECT,
    targetUnit: unitId,
  };
  reducer(action);
};
