...
type ClickReader = (e: React.MouseEvent) => {
  mouseX: number;
  fraction: number;
};
...
const useMousedownHandler = (
  divElmt: HTMLDivElement | null,
  clickReader: ClickReader,
  resetPanStateAnchor: (mouseX: number) => void
) => {
...
const useClickHandler = (
  divElmt: HTMLDivElement | null,
  clickReader: ClickReader,
  setCurrentTimeFraction: (fraction: number, opts: { event: React.MouseEvent }) => void
) => {
  const handler = useCallback(
    (e: React.MouseEvent) => {
      if (divElmt) {
...
const useTimeScrollEventHandlers = (
  leftMargin: number,
  panelWidth: number,
  panelWidthSeconds: number,
  divElmt: HTMLDivElement | null
) => {
  ...
  const { setPanUpdate, resetAnchor, startPan, clearPan, isPanning } = useTimeScrollPan(
    divElmt,
    secondsPerPixel,
    panTimeSelection
  );
