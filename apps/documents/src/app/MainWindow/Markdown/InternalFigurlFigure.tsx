...
const InternalFigurlFigure: FunctionComponent<Props> = ({ src, height }) => {
  ...
  return (
    <div>
      <FigurlFigureMenuBar src={undefined} visible={visible} setVisible={setVisible} />
      {
        ...
        >
          <InternalFigurlFigureInner src={src} height={height} />
        </div>
      }
    </div>
  );
  ...
  return (
    <ReactVisibilitySensor partialVisibility={true}>
      {({ isVisible }: { isVisible: boolean }) => {
        ...
        return isVisible || hasBeenVisible.current ? (
          <InternalFigurlFigureChild src={src} height={height} />
        ) : (
          <div style={{ position: 'relative', width: '100%', height }}>Waiting for visible</div>
        );
      }}
...
export const randomAlphaString = (num_chars: number) => {
  ...
  for (let i = 0; i < num_chars; i++) text += possible.charAt(Math.floor(Math.random() * possible.length));
  return text;
};
