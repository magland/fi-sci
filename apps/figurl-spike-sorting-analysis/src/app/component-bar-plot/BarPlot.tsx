...
import { BaseCanvas, Vec2, Vec4 } from '../figurl-canvas';
...
const BarPlot: FunctionComponent<Props> = ({ bars, ticks, verticalLines, xLabel, onSelectRect, width, height }) => {
  const { xMin, xMax } = useMemo(
    () => (bars.length > 0 ? { xMin: bars[0].xStart, xMax: bars[bars.length - 1].xEnd } : { xMin: 0, xMax: 1 }),
    [bars]
  );
  ...
  const handleSelectRect = useCallback(
    (r: Vec4, { ctrlKey, shiftKey }: { ctrlKey: boolean; shiftKey: boolean }) => {
  ...
  []);
  const { onMouseMove, onMouseDown, onMouseUp, onMouseLeave, paintDragSelectLayer } = useDragSelectLayer(
    width,
    height,
    handleSelectRect,
    handleClickPoint
