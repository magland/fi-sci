...
const RoiWidget: FunctionComponent<RoiWidgetProps> = ({width, height, nwbFile, timeIntervalsPath, roiClient, roiIndex, alignToVariables, windowRange, prefs}) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            for (const alignToVariable of alignToVariables) {
                const times = await nwbFile.getDatasetData(timeIntervalsPath + '/' + alignToVariable, {})
                if (!times) throw Error(`Unable to load ${timeIntervalsPath}/${alignToVariable}`)
                if (canceled) return
    ...
    return (
        <div style={{position: 'absolute', width, height, overflow: 'hidden'}}>
            <hr />
            <div style={{position: 'absolute', width, height: topBarHeight, fontSize: 18, marginLeft: 30}}>
                ROI {roiIndex}
            </div>
            {
                roiData && alignToTimes && alignToVariables ? (
                    alignToVariables.map((alignToVariable, i) => (
                        <div key={alignToVariable} style={{position: 'absolute', width: W, height: height - topBarHeight, top: topBarHeight, left: i * W}}>
                            <div style={{position: 'absolute', width: W, height: titleHeight, fontWeight: 'bold', textAlign: 'center'}}>
                                aligned to {alignToVariable}
                            </div>
                            <div style={{position: 'absolute', width: W, height: height - topBarHeight, top: titleHeight}}>
