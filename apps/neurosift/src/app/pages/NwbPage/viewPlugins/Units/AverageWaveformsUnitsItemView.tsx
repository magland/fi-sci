...
import { AverageWaveformData, AverageWaveformsViewData } from "../../../../package/view-average-waveforms/AverageWaveformsViewData"
...
const AverageWaveformsUnitsItemView: FunctionComponent<Props> = ({width, height, path, condensed}) => {
    const nwbFile = useNwbFile()
    if (!nwbFile) throw Error('Unexpected: nwbFile is null')    

    const [errorMessage, setErrorMessage] = useState<string | null>(null)
    const [data, setData] = useState<AverageWaveformsViewData | undefined | null>(undefined)
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if ((shape.length < 2) || (shape.length > 3)) {
                setErrorMessage(`waveform_mean dataset has unexpected shape: ${JSON.stringify(shape)}`)
                setData(null)
                return
            }
            if (shape.length === 2) {
            ...
            const waveformsData = await nwbFile.getDatasetData(`${path}/waveform_mean`, {})
            if (!waveformsData) {
            ...
            for (let i = 0; i < numUnits; i++) {
                const channelIds = Array.from({length: numChannels}, (_, i) => `index-${i}`)
                const waveform = create2DArray(numTimepoints, numChannels, (waveformsData as any as number[]).slice(i * numTimepoints * numChannels, (i + 1) * numTimepoints * numChannels))
...
const create2DArray = (rows: number, cols: number, data: number[]): number[][] => {
    const arr: number[][] = []
