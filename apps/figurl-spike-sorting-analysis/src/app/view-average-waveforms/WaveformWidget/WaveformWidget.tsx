...
import useWheelZoom from './sharedDrawnComponents/useWheelZoom';
...
const WaveformWidget: FunctionComponent<WaveformWidgetProps> = (props) => {
  ...
  const {
    ...
    pixelRadius,
    xMargin: xMarginBase,
  } = computeElectrodeLocations(width, height, electrodes, layoutMode, maxElectrodePixelRadius, { disableAutoRotate });
  ...
  const oneElectrodeWidth =
    layoutMode === 'geom' ? pixelRadius * 2 : width - xMargin - (showChannelIds ? 2 * pixelRadius : 0);
  const waveformPlot = (
    <WaveformPlot
