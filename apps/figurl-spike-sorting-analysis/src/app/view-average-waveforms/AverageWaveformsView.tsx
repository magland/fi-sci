...
import { getUnitColor, idToNum, INITIALIZE_UNITS, sortIds, useSelectedUnitIds } from '@fi-sci/context-unit-selection';
...
import {
  ...
  ViewToolbar,
} from '../view-units-table/ViewToolbar';
...
import { viewToolbarWidth } from '../view-units-table/ViewToolbar/ViewToolbar';
...
const AverageWaveformsView: FunctionComponent<Props> = ({ data, width, height }) => {
  ...
  const { selectedUnitIds, currentUnitId, orderedUnitIds, plotClickHandlerGenerator, unitIdSelectionDispatch } =
    useSelectedUnitIds();
  ...
  const plots: PGPlot[] = useMemo(
    () =>
      data.averageWaveforms
        .filter((a) => (toolbarOptions.onlyShowSelected ? selectedUnitIds.has(a.unitId) : true))
        .map((aw) => {
          const units: {
          ...
          }[] = [
            {
              ...
              waveformPercentiles:
                showWaveformStdev && !showOverlapping && aw.waveformPercentiles
                  ? subtractChannelMeansFromPercentiles(aw.waveformPercentiles, aw.waveform)
                  : undefined,
              waveformColor: getUnitColor(aw.unitId),
            },
          ];
    ...
    return [
      {
        type: 'button',
        callback: () => {
          setHorizontalStretchFactor((x) => x * 1.1);
        },
  ...
  const customToolbarActions = useMemo(() => {
    const amplitudeScaleToolbarEntries = AmplitudeScaleToolbarEntries({
      ampScaleFactor,
      setAmpScaleFactor,
    });
  ...
  return (
    <div ref={(elmt) => setupDivRef(elmt)} onWheel={handleWheel}>
      <Splitter width={width} height={height - bottomToolbarHeight} initialPosition={TOOLBAR_WIDTH} adjustable={false}>
        <ViewToolbar width={TOOLBAR_WIDTH} height={height} customActions={customToolbarActions} />
...
const combinePlotsForOverlappingView = (plots: PGPlot[]): PGPlot[] => {
  ...
  for (const plot of plots) {
    for (const id of plot.props.channelIds as (number | string)[]) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  plotProps.units = plots.map((p) => (p.props.units as any)[0]);
