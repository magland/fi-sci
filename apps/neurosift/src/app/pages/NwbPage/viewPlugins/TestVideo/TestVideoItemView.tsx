...
const TestVideoItemView: FunctionComponent<Props> = ({width, height, path}) => {
    ...
    const {videoWidth, videoHeight} = useMemo(() => {
        ...
        if (shape.length < 3) return {videoWidth: 0, videoHeight: 0, numFrames: 0}
...
const getVideoUrlForDataset = async (nwbFile: RemoteH5FileX, path: string): Promise<string> => {
