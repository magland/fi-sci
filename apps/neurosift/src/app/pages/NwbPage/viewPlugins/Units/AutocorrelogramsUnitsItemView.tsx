...
const AutocorrelogramsUnitsItemView: FunctionComponent<Props> = ({width, height, path, condensed}) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (shape.length !== 2) {
                setErrorMessage(`autocorrelogram dataset has unexpected shape: ${JSON.stringify(shape)}`)
                setData(null)
                return
            }
            ...
            if (numBins !== binEdgesSec.length - 1) {
                setErrorMessage(`Unexpected number of bins: ${numBins} != ${binEdgesSec.length - 1}`)
                setData(null)
                return
            }
            ...
            for (let i = 0; i < numUnits; i++) {
                const binCounts0 = autocorrelogramsData.slice(i * numBins, (i + 1) * numBins)
