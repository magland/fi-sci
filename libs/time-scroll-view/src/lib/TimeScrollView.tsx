...
export const useTimeScrollView = ({
  width,
  height,
  hideToolbar,
  leftMargin,
...
const TimeScrollView: FunctionComponent<Props> = ({
  width,
  height,
  onCanvasElement,
  gridlineOpts,
  ...
  const { currentTimeSec, visibleStartTimeSec, visibleEndTimeSec, zoomTimeSelection, panTimeSelectionPct } =
    useTimeSelection();
  ...
  const { margins, canvasWidth, canvasHeight, toolbarWidth } = useTimeScrollView({
    width,
    height: height2,
    hideToolbar,
    leftMargin,
  ...
  const pixelToTime = useMemo(() => {
    ...
    return (x: number) =>
      visibleStartTimeSec +
      ((x - margins.left) / (canvasWidth - margins.left - margins.right)) * (visibleEndTimeSec - visibleStartTimeSec);
  }, [canvasWidth, visibleStartTimeSec, visibleEndTimeSec, margins]);
  ...
  const currentTimePixels = useMemo(
    () => (currentTimeSec !== undefined ? timeToPixel(currentTimeSec) : undefined),
    [currentTimeSec, timeToPixel]
  );
  ...
  const { handleMouseDown, handleMouseUp, handleMouseLeave, handleMouseMove } = useTimeScrollEventHandlers(
    margins.left,
    canvasWidth - margins.left - margins.right,
    panelWidthSeconds,
    divElmt
