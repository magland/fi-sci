...
import { RemoteH5FileLindi, RemoteH5FileX } from "@fi-sci/remote-h5-file"
import { OpenInNew } from "@mui/icons-material"
import { FunctionComponent, useEffect, useMemo, useState } from "react"
import useRoute from "../../useRoute"
import { AssetsResponseItem, DandisetSearchResultItem, DandisetVersionInfo } from "../DandiPage/DandiBrowser/types"
import { assetUrlForPath, useDandisetVersionInfo, useQueryAssets, useQueryDandiset } from "../DandisetPage/DandisetViewFromDendro/DandisetView"
...
import EphysSummaryItemView, { Expandable } from "../NwbPage/viewPlugins/Ephys/EphysSummaryItemView"
...
const EphysSummaryPluginPage: FunctionComponent<EphysSummaryPluginPageProps> = ({width, height}) => {
    const { route, setRoute } = useRoute()
    if (route.page !== 'plugin') throw Error('Unexpected route for PluginPage: ' + route.page)
    if (route.plugin !== 'EphysSummary') throw Error('Unexpected plugin for PluginPage: ' + route.plugin)
    ...
    return (
        <LazyPlotlyPlotContext.Provider value={lazyPlotlyPlotContextValue}>
            <div style={{position: 'relative', left: 10, width: width - 20, height, overflow: 'auto'}}>
                <h2>
                    Ephys Summary for Dandiset {dandisetId}
                </h2>
                {
                ...
                {
                    dandisetId && allEphysAssets?.map((asset, i) => (
                        <Expandable
                            title={
                                <>
                                    {asset.path}
                                    ...
                                    <SmallIconButton
                                        ...
                                        onClick={() => {
                                            setRoute({
                                                page: 'nwb',
                                                url: [assetUrlForPath(asset.path, allAssets || [], useStaging)],
                                                dandisetId,
                                                dandisetVersion,
                        ...
                        >
                            <EphysSummaryAssetView
                                ...
                                assetUrl={assetUrlForPath(asset.path, allAssets || [], useStaging)}
...
const useRawEphysPaths = (nwbFile: RemoteH5FileX | undefined) => {
    const [rawEphysPaths, setRawEphysPaths] = useState<string[] | undefined>(undefined)
    useEffect(() => {
        if (!nwbFile) return
        let canceled = false
        ;(async () => {
            ...
            for (const b of a.subgroups) {
                if (b.attrs['neurodata_type'] === 'ElectricalSeries') {
                    ...
                    if (!g) continue
                    const startTimeDs = g.datasets.find(ds => ds.name === 'starting_time')
...
const EphysSummaryAssetView: FunctionComponent<EphysSummaryAssetViewProps> = ({width, dandisetId, assetPath, assetUrl, assetId}) => {
...
const EphysSummaryElectricalSeriesView: FunctionComponent<EphsSummaryElectricalSeriesViewProps> = ({width, electricalSeriesPath}) => {
    return (
        <div>
