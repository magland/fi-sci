...
const isNH5HeaderDataset = (x: any): x is NH5HeaderDataset => {
    return validateObject(x, {
        path: isString,
        attrs: () => (true),
        dtype: isOneOf(['int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'float32', 'float64'].map(x => (isEqualTo(x)))),
...
export class RemoteNH5FileClient {
    ...
    async getGroup(path: string): Promise<RemoteNH5Group | undefined> {
        ...
        const subgroups = this.header.groups.filter(g => (g.path.startsWith(path + '/'))).filter(g => (g.path.split('/').length === path.split('/').length + 1)).map(g => ({
            name: g.path.split('/').slice(-1)[0],
            path: g.path,
            attrs: g.attrs
        }))
        const datasets = this.header.datasets.filter(d => (d.path.startsWith(path + '/'))).filter(d => (d.path.split('/').length === path.split('/').length + 1)).map(d => ({
