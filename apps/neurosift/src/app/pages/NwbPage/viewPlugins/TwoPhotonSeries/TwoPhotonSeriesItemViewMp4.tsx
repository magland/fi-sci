...
const TwoPhotonSeriesItemViewMp4: FunctionComponent<TwoPhotonSeriesItemViewMp4Props> = ({ width, height, path }) => {
    ...
    useEffect(() => {
        ...
        if (!(nwbFile instanceof RemoteH5FileLindi)) {
            throw Error('TwoPhotonSeriesItemViewMp4 only works with RemoteH5FileLindi')
        }
        const client = nwbFile._lindiFileSystemClient
            ; (async () => {
                ...
                for (let i = 1; i < shape.length; i++) {
                    if (shape[i] !== chunks[i]) throw Error('Chunks must match shape for dimensions > 0')
                }
                const chunkKey = shape.length === 4 ? '0.0.0.0' : shape.length === 3 ? '0.0.0' : undefined
                if (!chunkKey) throw Error('Unsupported shape length')
                const buf = await client.readBinary(pathWithoutBeginningSlash + '/data/' + chunkKey, {})
                setMp4Buf(buf)
    ...
    return (
        <div>
            <div style={{ filter: `brightness(${brightness}%) contrast(${contrast}%)` }}>
                {dataUrl && size &&
            ...
            <BrightnessContrastSelector brightness={brightness} setBrightness={setBrightness} contrast={contrast} setContrast={setContrast} />
        </div>
    )
}

const BrightnessContrastSelector: FunctionComponent<{ brightness: number, setBrightness: (b: number) => void, contrast: number, setContrast: (c: number) => void }> = ({ brightness, setBrightness, contrast, setContrast }) => {
    // use a couple sliders
    return (
        <div>
            <label htmlFor="brightness-slider">Brightness:</label>
            <input id="brightness-slider" type="range" min="0" max="300" value={brightness} onChange={e => setBrightness(parseInt(e.target.value))} />

            <label htmlFor="contrast-slider">Contrast:</label>
            <input id="contrast-slider" type="range" min="0" max="300" value={contrast} onChange={e => setContrast(parseInt(e.target.value))} />
        </div>
    )
}
