...
export const computeElectrodesFromIdsAndLocations = (
  ...
  if (channelLocations && (Object.keys(locs).length !== ids.length || ids.some((id) => !locs[`${id}`]))) {
...
const computeDataToPixelTransform = (
  ...
  const yrow = [
    0,
    -scaleFactor,
    (height + scaleFactor * (electrodeLayoutBoundingBox.ymin + electrodeLayoutBoundingBox.ymax)) / 2,
  ];
...
export const computeElectrodeLocations = (
  ...
  const { electrodes: normalizedElectrodes, rotated } = normalizeElectrodeLocations(
    canvasWidth,
    canvasHeight,
    electrodes,
    {
