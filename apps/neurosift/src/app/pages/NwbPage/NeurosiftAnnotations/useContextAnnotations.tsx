...
import { FunctionComponent, PropsWithChildren, createContext, useCallback, useContext, useEffect, useMemo, useState } from "react"
...
import { AddAnnotationRequest, DeleteAnnotationRequest, GetAnnotationsRequest, NeurosiftAnnotation, isGetAnnotationsResponse } from "./types"

type ContextAnnotationsContextType = {
    contextAnnotations?: NeurosiftAnnotation[]
    refreshContextAnnotations: () => void
    addContextAnnotation: (annotationType: string, annotation: any) => Promise<void>
...
const ContextAnnotationsContext = createContext<ContextAnnotationsContextType>(defaultContextAnnotationsContext)
...
export const SetupContextAnnotationsProvider: FunctionComponent<PropsWithChildren<SetupContextAnnotationsProviderProps>> = ({ children }) => {
    ...
    const fetchContextAnnotations = useMemo(() => (async (): Promise<NeurosiftAnnotation[]> => {
        ...
        return annotations.sort((a, b) => a.timestampCreated - b.timestampCreated)
    }), [assetId, dandisetId])
    ...
    const addContextAnnotation = useCallback(async (annotationType: string, annotation: any) => {
        if (!neurosiftAnnotationsAccessToken) {
            console.warn('Cannot setContextAnnotations because neurosiftAnnotationsAccessToken is not set')
            return
        }
        if (!neurosiftAnnotationsUserId) {
            console.warn('Cannot setContextAnnotations because neurosiftAnnotationsUserId is not set')
            return
        }
        if (!dandisetId) {
            console.warn('Cannot setContextAnnotations because dandisetId is not set')
            return
        }
    ...
    return (
        <ContextAnnotationsContext.Provider value={{contextAnnotations: contextAnnotations, refreshContextAnnotations: refreshContextAnnotations, addContextAnnotation: addContextAnnotation, removeContextAnnotation: removeContextAnnotation}}>
            {children}
