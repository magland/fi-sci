...
const computePlotSeries = (resolvedSeries: ResolvedSeries[]): PlotSeries[] => {
  ...
  resolvedSeries.forEach((rs) => {
    const tt = rs.t;
    const yy = rs.y;
    let filteredTimeIndices: number[] = tt.flatMap((t: number, ii: number) =>
      visibleStartTimeSec <= t && t <= visibleEndTimeSec ? ii : []
    );
...
const applyLineAttributes = (
  ...
  attributes: { [key: string]: string | number | number[] | number[] }
) => {
