...
import { BaseCanvas, DragAction, DragCanvas, Vec2, handleMouseDownIfDragging, handleMouseMoveIfDragging, handleMouseUpIfDragging } from '../../../figurl-canvas'
...
const ElectrodeGeometry = (props: WidgetProps) => {
    ...
    const handleMouseUp = useCallback((e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        if (state.dragState.isActive) {
            ...
            handleMouseUpIfDragging(e, {nextDragStateUpdate, nextFrame, reducer: dispatchState, reducerOtherProps: {type: 'DRAGUPDATE', /*selectedElectrodeIds: selectedElectrodeIds*/ selectedElectrodeIds: []}})
        } else {
    ...
    }, [state.dragState.isActive, getEventPointWithAffineTransform])
    ...
    const canvas = useMemo(() => {
        ...
        return <BaseCanvas
            ...
            drawData={data}
        />
    }, [width, height, state2.convertedElectrodes, state.hoveredElectrodeId, state.draggedElectrodeIds, state2.pixelRadius, props.showLabels, offsetLabels, props.layoutMode, state.xMarginWidth, colors, affineTransform])
    ...
    const svg = useMemo(() => {
        return USE_SVG && <SvgElectrodeLayout 
            ...
            colors={colors}
        />
    }, [state.convertedElectrodes, state.hoveredElectrodeId, state.draggedElectrodeIds, state.pixelRadius, props.showLabels, offsetLabels, props.layoutMode, state.xMarginWidth, width, height, colors])
