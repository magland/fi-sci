...
class ClonedRepo {
    ...
    static async load(a: {url: string, branch: string, setStatus: (status: string) => void}): Promise<ClonedRepo> {
        const X = new ClonedRepo(a)
        await X.initialize(a.setStatus)
        return X
    }
    async clear() {
    ...
    async getSymlinkTarget(p: string): Promise<string> {
        const fullPath = joinPaths(this.#dir, p)
        const a = await fs.promises.readlink(fullPath)
        return a
    }
}
