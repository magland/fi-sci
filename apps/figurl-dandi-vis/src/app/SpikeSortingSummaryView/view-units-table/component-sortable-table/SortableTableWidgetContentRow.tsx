...
const SortableTableWidgetContentRow: FunctionComponent<RowProps> = (props: RowProps) => {
  ...
  return (
    <TableRow key={rowId} className={current ? 'currentRow' : selected ? 'selectedRow' : ''}>
      {onClick && (
        <TableCell key="_checkbox">
          <SortableTableWidgetCheckbox rowId={rowId} selected={selected} onClick={onClick} isDisabled={isDisabled} />
        </TableCell>
      )}
      {content}
    </TableRow>
