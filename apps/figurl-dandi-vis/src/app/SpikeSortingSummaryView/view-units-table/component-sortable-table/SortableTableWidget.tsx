...
import { allUnitSelectionState, voidClickHandler } from '@fi-sci/context-unit-selection';
...
const SortableTableWidget: FunctionComponent<SortableTableProps> = (props) => {
  ...
  const _visibleUnitIds = useMemo(() => {
    return visibleUnitIds && visibleUnitIds.length > 0 ? visibleUnitIds : orderedUnitIds;
    ...
    rows.forEach((row) => {
      const columnValues = columns.map((column) => (
        <TableCell key={column.columnName}>
          <div title={column.tooltip}>{column.dataElement(row.data[column.columnName].value)}</div>
        </TableCell>
      ));
