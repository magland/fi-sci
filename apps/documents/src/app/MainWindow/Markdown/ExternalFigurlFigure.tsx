...
const ExternalFigurlFigure: FunctionComponent<Props> = ({ src, height }) => {
  ...
  return (
    <div>
      <FigurlFigureMenuBar src={src} visible={visible} setVisible={setVisible} />
      {
        ...
        >
          <ExternalFigurlFigureInner src={src} height={height} />
        </div>
      }
    </div>
  );
    ...
    <ReactVisibilitySensor partialVisibility={true}>
      {({ isVisible }: { isVisible: boolean }) => {
        ...
        return isVisible || hasBeenVisible.current ? (
          <iframe title="external figurl figure" src={src + '&hide=1'} width="100%" height={height} frameBorder={0} />
        ) : (
