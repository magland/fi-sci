...
export const useTimeSelection = () => {
  ...
  const reportTotalTimeRange = useCallback(
  ...
  const panTimeSelection = useCallback(
    (deltaSec: number) => {
      dispatch({
  ...
  const zoomTimeSelection = useCallback(
    (factor: number, anchorTimeSec?: number) => {
      dispatch({
  ...
  const panTimeSelectionPct = useCallback(
    (pct: number) => {
      dispatch({
